package org.mystock.repository;

import org.mystock.entity.ColorEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;
import org.springframework.stereotype.Repository;

import java.util.List;

@Repository
public interface ColorRepository extends JpaRepository<ColorEntity, Long> {

	@Query("SELECT c FROM ColorEntity c WHERE UPPER(c.colorName) LIKE UPPER(:name) order by c.colorName")
	List<ColorEntity> findByNameIgnoreCaseLike(@Param("name") String name);

	@Query("SELECT c FROM ColorEntity c WHERE UPPER(c.colorName) = UPPER(:name) order by c.colorName")
	ColorEntity findByNameIgnoreCase(@Param("name") String name);
}