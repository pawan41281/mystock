...
public interface ContractorChallanRepository extends JpaRepository<ContractorChallanEntity, Long> {
    ...
    @Query("""
            select c 
            from 
            ContractorChallanEntity c 
            inner join c.challanItems challanItems
            where
            c.challanType = :challanType
            and 
            c.contractor.id = :contractorId 
            and 
            challanItems.quality.id = :qualityId 
            and 
    ...
    @Query(value = """
            SELECT c.* 
            FROM contractor_challan_info c
            INNER JOIN contractor_challan_item_info ci 
                ON ci.challan_id = c.id
            WHERE c.challan_type = :challanType
